...
using static Reports.Application.Contracts.Messages.ReceiveEmailMessage;

namespace Reports.Application.Handlers.Messages;

public class ReceiveEmailMessageHandler : IRequestHandler<Command, Response>
{
    private readonly IReportsDatabaseContext _context;

    public ReceiveEmailMessageHandler(IReportsDatabaseContext context)
    {
    ...
    {
        var newMessage = new EmailMessage(request.Text, request.Topic, request.SenderEmail);
        MessageSource messageSource = await _context.Sources.GetEntityByIdAsync(request.SourceId, cancellationToken);

        if (messageSource is not EmailMessageSource emailMessageSource)
            throw new EntityNotFoundException<EmailMessageSource>(request.SourceId);

        emailMessageSource.ReceiveMessage(newMessage);
